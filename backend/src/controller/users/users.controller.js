...
const consultarPorCodigo = async function (req, res) {
  console.log("consultar 1 usuario por codigo controller id: "+req.params.filtro);
  try {
...
const login = async function (req, res) {
  ...
  try {
    ...
    if (userDB.length > 0 && userDB[0].length > 0 && !userDB[0][0].deleted) {
...
const logout = async function (req, res) {
  try {
    ...
    const usersDB = await sequelize.query(
      "UPDATE users SET token = null WHERE id = " + req.body.id + ""
    );
