const { sequelize } = require("../../connection");
  ...
  // } = require("../../model/themes_properties");
...
const consultarPorCodigo = async function (req, res) {
  ...
  try {
    const themesPropertiesModelResult =
      await themesPropertiesService.busquedaPorCodigo(req.params.idTema, req.params.id);
...
const eliminar = async function (req, res) {
  ...
  try {
    await themesPropertiesService.eliminar(req.params.id, req.params.idTema);
    res.json({
      success: true,
    });
  } catch (error) {
